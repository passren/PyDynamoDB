...
class TestUtil:
    def test_parse_limit_expression(self):
        assert parse_limit_expression("SELECT * FROM TEST limiT 10") == (
            "SELECT * FROM TEST ",
            10,
        )
        assert parse_limit_expression("SELECT * FROM TEST limiT   10000") == (
            "SELECT * FROM TEST ",
            10000,
        )
        ...
        assert parse_limit_expression("SELECT limi FROM TEST  WHERE X = 1 LIMIT  ") == (
